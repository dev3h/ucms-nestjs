...
export class UcmsMigration1733338076707 implements MigrationInterface {
    name = 'UcmsMigration1733338076707'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`logs\` (\`id\` int NOT NULL AUTO_INCREMENT, \`level\` tinyint NOT NULL COMMENT '0: debug, 1: info, 2: warning, 3: error, 4: critical', \`message\` text NOT NULL, \`timestamp\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`user_id\` bigint NULL, \`module\` varchar(255) NULL, \`function_name\` varchar(255) NULL, \`status_code\` int NULL, \`ip_address\` varchar(45) NULL, \`user_agent\` text NULL, \`stack_trace\` text NULL, \`additional_data\` json NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`device_sessions\` ADD CONSTRAINT \`FK_75adfe32963071556b9de6c9501\` FOREIGN KEY (\`system_id\`) REFERENCES \`systems\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`device_sessions\` DROP FOREIGN KEY \`FK_75adfe32963071556b9de6c9501\``);
